...
Calendar get_calendar_info_no_check(int year, int month, int day, int hour, int minute, int second) {
    ...
    // 时间信息
    calendar.calendar.time.hour = hour;
    calendar.calendar.time.minute = minute;
    calendar.calendar.time.second = second;

    // 公历信息
    calendar.calendar.solar.year = year;
    calendar.calendar.solar.month = month;
    calendar.calendar.solar.day = day;
    calendar.calendar.solar.week = get_day_of_week(year, month, day);
    calendar.calendar.solar.daysOfMonth = get_days_of_month(year, month);
    calendar.calendar.solar.daysOfYear = is_leap_year(year) ? 366 : 365;
    calendar.calendar.solar.dayOfYear = get_day_ordinal(year, month, day) + 1;
    ...
    // 获取节气本月节气的日期和年内序数
    get_jieqi_data(calendar.calendar.solar.year, calendar.calendar.solar.month, jieqi_date, jieqi_ordinal);
    if (calendar.calendar.solar.day < jieqi_date[0]) { // 1的位置
        calendar.calendar.solarTerms.solarTerm = (index + 22) % 24 + 1; // 上一个
    } else if (calendar.calendar.solar.day >= jieqi_date[1]) { // 3的位置
        calendar.calendar.solarTerms.solarTerm = index + 1; // 下一个
    } else { // 2的位置
        calendar.calendar.solarTerms.solarTerm = index; // 当前
    }
    // 下一个节气
    calendar.calendar.solarTerms.nextSolarTerm = (calendar.calendar.solarTerms.solarTerm % 24) + 1;
    if (calendar.calendar.solar.day == jieqi_date[0] || calendar.calendar.solar.day == jieqi_date[1]) {
        calendar.calendar.solarTerms.isToday = true;
    }
    // 计算剩余天数
    if (calendar.calendar.solar.day < jieqi_date[0]) { // 1的位置
        calendar.calendar.solarTerms.nextSolarTermRemainDays = jieqi_date[0] - calendar.calendar.solar.day;
    } else if (jieqi_date[0] <= calendar.calendar.solar.day && calendar.calendar.solar.day < jieqi_date[1]) {
        calendar.calendar.solarTerms.nextSolarTermRemainDays = jieqi_date[1] - calendar.calendar.solar.day;
    } else {
        current_ordinal = calendar.calendar.solar.dayOfYear - 1;
        month = ((calendar.calendar.solarTerms.nextSolarTerm + 1) / 2) % 12 + 1;//将下一个节气的序号转换到月份
        if (month == 1) {
            if (calendar.calendar.solar.year + 1 < end_year) {
                get_jieqi_data(calendar.calendar.solar.year + 1, month, jieqi_date, jieqi_ordinal);
                calendar.calendar.solarTerms.nextSolarTermRemainDays = 31 - day + jieqi_date[0];
            } else if (calendar.calendar.solar.year + 1 == end_year) {
                calendar.calendar.solarTerms.nextSolarTermRemainDays = calendar.calendar.solar.daysOfYear - current_ordinal + 4;
            }
        } else {
            get_jieqi_data(calendar.calendar.solar.year, month, jieqi_date, jieqi_ordinal);
            calendar.calendar.solarTerms.nextSolarTermRemainDays = jieqi_ordinal[0] - current_ordinal;
        }
    }

    //农历信息
    calendar.calendar.lunar.chineseCalendarYear = calendar.calendar.solar.year;
    calendar.calendar.lunar.isChineseCalendarLeapMonth = false;//默认不是闰月
    current_ordinal = calendar.calendar.solar.dayOfYear - 1;//获取日期的年内序数
    lunar_new_year = get_lunar_new_year_ordinal(calendar.calendar.lunar.chineseCalendarYear);//获取春节的年内序数
    days_since_lunar_new_year = current_ordinal - lunar_new_year;//距离农历新年的天数
    //获取月份天数, 数组从上一年十一月开始到今年(闰)十二月, 包含闰月
    lunar_expand_dx(calendar.calendar.lunar.chineseCalendarYear, days_of_lunar_month);
    pre_leap_month = get_lunar_leap_month(calendar.calendar.lunar.chineseCalendarYear - 1);//获取上一年的闰月
    if (pre_leap_month == 0) {//上一年没有闰月
        leap_month = get_lunar_leap_month(calendar.calendar.lunar.chineseCalendarYear);//查询今年的闰月
    }
    ...
    if (current_ordinal < lunar_new_year) {//年内序数早于农历新年
        calendar.calendar.lunar.chineseCalendarYear -= 1;//农历年减1
        ...
        calendar.calendar.lunar.isDXYue = days_of_lunar_month[index] == 30;
        if (pre_leap_month < 11) {//如果上一年十一月或十二月不存在闰月
            calendar.calendar.lunar.chineseCalendarMonth = 11 + index;
            calendar.calendar.lunar.isChineseCalendarLeapMonth = false;
        } else if (pre_leap_month == 11) {//闰十一月
            if (index == 0) {//十一月(即在闰月之前)
                calendar.calendar.lunar.chineseCalendarMonth = 11 + index;//转换到月份
                calendar.calendar.lunar.isChineseCalendarLeapMonth = false;
            } else {
                calendar.calendar.lunar.chineseCalendarMonth = 10 + index;//转换到月份
                calendar.calendar.lunar.isChineseCalendarLeapMonth = calendar.calendar.lunar.chineseCalendarMonth == pre_leap_month;
            }
        ...
        calendar.calendar.lunar.chineseCalendarDay = days_since_lunar_new_year;
    } else {
        ...
        if (pre_leap_month > 10) {
            calendar.calendar.lunar.chineseCalendarMonth = index - 2;
            calendar.calendar.lunar.isChineseCalendarLeapMonth = false;
        } else {
            if (0 < leap_month && leap_month <= index - 2) {
                calendar.calendar.lunar.chineseCalendarMonth = index - 2;
                calendar.calendar.lunar.isChineseCalendarLeapMonth =
                        calendar.calendar.lunar.chineseCalendarMonth == leap_month;
            } else {
                calendar.calendar.lunar.chineseCalendarMonth = index - 1;
                calendar.calendar.lunar.isChineseCalendarLeapMonth = false;
            }
        }
        calendar.calendar.lunar.chineseCalendarDay = days_since_lunar_new_year;
    }
    calendar.calendar.lunar.chineseCalendarDay += 1;//索引转换到数量
    calendar.calendar.lunar.isDXYue = days_of_lunar_month[index] == 30;//days_of_lunar_month[index]为当前农历月的天数
    calendar.calendar.lunar.celestialStem = (calendar.calendar.lunar.chineseCalendarYear - 4) % 10 + 1; // 天干
    calendar.calendar.lunar.terrestrialBranch = (calendar.calendar.lunar.chineseCalendarYear - 4) % 12 + 1; // 地支
    calendar.calendar.lunar.chineseZodiac = calendar.calendar.lunar.terrestrialBranch; // 生肖

    //四柱-年干支
    four_pillars_year = calendar.calendar.lunar.chineseCalendarYear;
    if (calendar.calendar.lunar.chineseCalendarYear == calendar.calendar.solar.year && current_ordinal < lichun_order) {
        four_pillars_year -= 1;
    }
    calendar.calendar.fourPillars.year.celestialStem = (four_pillars_year - 4) % 10 + 1;
    calendar.calendar.fourPillars.year.terrestrialBranch = (four_pillars_year - 4) % 12 + 1;

    //四柱-月干支
    //将节气的序号转换到以立春为正月初一的月份
    four_pillars_month = calendar.calendar.solarTerms.solarTerm / 2 + calendar.calendar.solarTerms.solarTerm % 2;
    switch (calendar.calendar.fourPillars.year.celestialStem) {
        case 1:
        case 6:
            calendar.calendar.fourPillars.month.celestialStem = (2 + four_pillars_month - 1) % 10 + 1;
            calendar.calendar.fourPillars.month.terrestrialBranch = (2 + four_pillars_month - 1) % 12 + 1;
            break;
        case 2:
        case 7:
            calendar.calendar.fourPillars.month.celestialStem = (4 + four_pillars_month - 1) % 10 + 1;
            calendar.calendar.fourPillars.month.terrestrialBranch = (2 + four_pillars_month - 1) % 12 + 1;
            break;
        case 3:
        case 8:
            calendar.calendar.fourPillars.month.celestialStem = (6 + four_pillars_month - 1) % 10 + 1;
            calendar.calendar.fourPillars.month.terrestrialBranch = (2 + four_pillars_month - 1) % 12 + 1;
            break;
        case 4:
        case 9:
            calendar.calendar.fourPillars.month.celestialStem = (8 + four_pillars_month - 1) % 10 + 1;
            calendar.calendar.fourPillars.month.terrestrialBranch = (2 + four_pillars_month - 1) % 12 + 1;
            break;
        case 5:
        case 10:
            calendar.calendar.fourPillars.month.celestialStem = (0 + four_pillars_month - 1) % 10 + 1;
            calendar.calendar.fourPillars.month.terrestrialBranch = (2 + four_pillars_month - 1) % 12 + 1;
            break;
        default:
            calendar.calendar.fourPillars.month.celestialStem = calendar.calendar.fourPillars.month.terrestrialBranch = -1;
    }

    //四柱-日干支
    calendar.calendar.fourPillars.day.celestialStem = get_solar_new_year_solar_terms(year);
    calendar.calendar.fourPillars.day.terrestrialBranch = get_solar_new_year_terrestrial_branch(year);
    switch (calendar.calendar.solar.month) {
        case 2:
        case 6:
            calendar.calendar.fourPillars.day.celestialStem += 1;
            calendar.calendar.fourPillars.day.terrestrialBranch += 7;
            break;
        case 3:
            calendar.calendar.fourPillars.day.celestialStem -= 1;
            calendar.calendar.fourPillars.day.terrestrialBranch -= 1;
            break;
        case 4:
            calendar.calendar.fourPillars.day.celestialStem += 0;
            calendar.calendar.fourPillars.day.terrestrialBranch += 6;
            break;
        case 5:
            calendar.calendar.fourPillars.day.celestialStem += 0;
            calendar.calendar.fourPillars.day.terrestrialBranch += 0;
            break;
        case 7:
            calendar.calendar.fourPillars.day.celestialStem += 1;
            calendar.calendar.fourPillars.day.terrestrialBranch += 1;
            break;
        case 8:
            calendar.calendar.fourPillars.day.celestialStem += 2;
            calendar.calendar.fourPillars.day.terrestrialBranch += 8;
            break;
        case 9:
            calendar.calendar.fourPillars.day.celestialStem += 3;
            calendar.calendar.fourPillars.day.terrestrialBranch += 3;
            break;
        case 10:
            calendar.calendar.fourPillars.day.celestialStem += 3;
            calendar.calendar.fourPillars.day.terrestrialBranch += 9;
            break;
        case 11:
            calendar.calendar.fourPillars.day.celestialStem += 4;
            calendar.calendar.fourPillars.day.terrestrialBranch += 4;
            break;
        case 12:
            calendar.calendar.fourPillars.day.celestialStem += 4;
            calendar.calendar.fourPillars.day.terrestrialBranch += 10;
            break;
        default:
            break;
    }
    if (calendar.calendar.solar.month > 2 && is_leap_year(calendar.calendar.solar.year)) {
        calendar.calendar.fourPillars.day.celestialStem += 1;
        calendar.calendar.fourPillars.day.terrestrialBranch += 1;
    }
    if (calendar.calendar.time.hour == 23 || calendar.calendar.time.hour == 0) {
        calendar.calendar.fourPillars.day.celestialStem += 1;
        calendar.calendar.fourPillars.day.terrestrialBranch += 1;
    }
    calendar.calendar.fourPillars.day.celestialStem =
            (calendar.calendar.fourPillars.day.celestialStem + day - 1) % 10 + 1;
    calendar.calendar.fourPillars.day.terrestrialBranch =
            (calendar.calendar.fourPillars.day.terrestrialBranch + day - 1) % 12 + 1;

    //四柱-时干支
    switch (calendar.calendar.fourPillars.day.celestialStem) {
        case 1:
        case 6:
            calendar.calendar.fourPillars.hour.celestialStem = (0 + (calendar.calendar.time.hour + 1) / 2) % 10 + 1;
            break;
        case 2:
        case 7:
            calendar.calendar.fourPillars.hour.celestialStem = (2 + (calendar.calendar.time.hour + 1) / 2) % 10 + 1;
            break;
        case 3:
        case 8:
            calendar.calendar.fourPillars.hour.celestialStem = (4 + (calendar.calendar.time.hour + 1) / 2) % 10 + 1;
            break;
        case 4:
        case 9:
            calendar.calendar.fourPillars.hour.celestialStem = (6 + (calendar.calendar.time.hour + 1) / 2) % 10 + 1;
            break;
        case 5:
        case 10:
            calendar.calendar.fourPillars.hour.celestialStem = (8 + (calendar.calendar.time.hour + 1) / 2) % 10 + 1;
            break;
        default:
            calendar.calendar.fourPillars.hour.celestialStem = calendar.calendar.fourPillars.hour.terrestrialBranch = -1;
    }
    calendar.calendar.fourPillars.hour.terrestrialBranch = ((calendar.calendar.time.hour + 1) / 2) % 12 + 1;
    return calendar;
}
#endif
    ...
    if (calendar.solar.day < jieqi_date[0]) { // 1的位置
        ...
        if (month == 1) {
            if (calendar.solar.year + 1 < end_year) {
                ...
                calendar.solarTerms.nextSolarTermRemainDays = calendar.solar.daysOfYear - current_ordinal + 4;
            }
        } else {
    ...
    if (current_ordinal < lunar_new_year) {//年内序数早于农历新年
        ...
        if (pre_leap_month < 11) {//如果上一年十一月或十二月不存在闰月
            ...
            if (index == 0) {//十一月(即在闰月之前)
                ...
                calendar.lunar.isChineseCalendarLeapMonth = calendar.lunar.chineseCalendarMonth == pre_leap_month;
            }
        } else if (pre_leap_month == 12) {//闰十二月
            if (index < 2) {//十一月(即在闰月之前)
                calendar.lunar.chineseCalendarMonth = 11 + index;//转换到月份
...
#if defined(USE_UNION) && USE_UNION == 1
    uint64_t hash = default_murmurhash3_64(calendarInfo.calendar.data, sizeof(Calendar), seed);
#endif
